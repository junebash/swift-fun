...
# Local release script for swift-fun
...
Transform this raw changelog into release notes matching the swift-fun style.
...
2. Group changes by module when applicable (Either, SequenceBuilder, StdPlus, Box, AsyncPlus, IO)
3. Use clear headers like '### New Modules', '### Module Additions', '### Bug Fixes'
4. Write brief but informative descriptions
5. Highlight any breaking changes prominently at the top
6. Do NOT include the version header (## vX.Y.Z) - just the content
7. Do NOT include horizontal rules (---)
8. If there are very few changes, keep the notes concise
